...
Usage:
    python3 stock_data_fetcher.py --stocks "600519,TSLA,HK00700" [--days 120] [--output csv]
...
# Output mode (set by main)
_LOG_FILE = None  # type: ignore


def _log(msg):
    """Log to stderr (default) or to a log file (CSV mode)."""
    dest = open(_LOG_FILE, "a") if _LOG_FILE else sys.stderr
...
def write_csv_output(results: list, output_dir: str, analysis_date: str = "") -> None:
    """Write OHLCV CSV per stock + summary CSV to output_dir."""
    import csv
    os.makedirs(output_dir, exist_ok=True)

    # Per-stock OHLCV CSV
    ohlcv_fields = ["code", "date", "open", "high", "low", "close", "volume", "amount", "pct_chg"]
    for r in results:
        code = r["code"]
        ohlcv = r.get("recent_bars", [])
        if not ohlcv:
            continue
        # Inject code into each row
        for row in ohlcv:
            row["code"] = code
        path = os.path.join(output_dir, f"{code}_ohlcv.csv")
        with open(path, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=ohlcv_fields, extrasaction="ignore")
            w.writeheader()
            w.writerows(ohlcv)
        _log(f"Wrote {len(ohlcv)} bars → {path}")

    # Summary CSV — one row per stock
    summary_path = os.path.join(output_dir, "analysis_summary.csv")
    summary_fields = [
        "analysis_date", "code", "name", "market", "data_source", "total_bars",
        "price", "change_pct",
        "MA5", "MA10", "MA20", "MA60", "ma_alignment",
        "MACD_DIF", "MACD_DEA", "MACD_hist", "MACD_signal",
        "RSI6", "RSI12", "RSI24", "RSI_zone",
        "vol_ratio", "vol_trend",
    ...
    with open(summary_path, "w", newline="") as f:
        ...
        for r in results:
            ...
            row = {
                ...
                "MA10": ind.get("ma", {}).get("MA10"),
                "MA20": ind.get("ma", {}).get("MA20"),
                "MA60": ind.get("ma", {}).get("MA60"),
                "ma_alignment": ind.get("ma", {}).get("alignment", ""),
                "MACD_DIF": ind.get("macd", {}).get("DIF"),
                "MACD_DEA": ind.get("macd", {}).get("DEA"),
                "MACD_hist": ind.get("macd", {}).get("hist"),
                "MACD_signal": ind.get("macd", {}).get("signal", ""),
                "RSI6": ind.get("rsi", {}).get("RSI6"),
                "RSI12": ind.get("rsi", {}).get("RSI12"),
                "RSI24": ind.get("rsi", {}).get("RSI24"),
                "RSI_zone": ind.get("rsi", {}).get("zone", ""),
                "vol_ratio": ind.get("volume", {}).get("vol_ratio"),
                "vol_trend": ind.get("volume", {}).get("trend", ""),
                "bias_ma5": ind.get("bias", {}).get("bias_ma5"),
                "bias_ma10": ind.get("bias", {}).get("bias_ma10"),
                "bias_ma20": ind.get("bias", {}).get("bias_ma20"),
                "support_ma5": ind.get("support", {}).get("support_ma5", False),
                "support_ma10": ind.get("support", {}).get("support_ma10", False),
                "trend_score": sc.get("total"),
                "trend_signal": sc.get("signal_cn", ""),
            }
            w.writerow(row)
...
def main():
    ...
    parser.add_argument("--days", type=int, default=120, help="History trading days (ignored if --start-date is set)")
    parser.add_argument("--start-date", default=None, help="Start date YYYY-MM-DD (overrides --days)")
    parser.add_argument("--end-date", default=None, help="End date YYYY-MM-DD (default: today)")
    parser.add_argument(
        "--output", choices=["stderr", "csv"], default="stderr",
        help="Output mode: stderr (log to stderr) | csv (OHLCV CSV + summary CSV)",
    )
    parser.add_argument("--output-dir", default="./output", help="CSV output directory (default: ./output)")
    args = parser.parse_args()

    # Resolve effective days from date range or --days
    if args.start_date:
        try:
            ...
            # Rough estimate: ~60% of calendar days are trading days
            effective_days = max(10, int(calendar_days * 0.6))
            _log(f"Date range {args.start_date} → {end.strftime('%Y-%m-%d')} ({calendar_days} calendar days, ~{effective_days} trading days)")
        except ValueError as e:
            _log(f"Invalid date format: {e}, falling back to --days={args.days}")
            effective_days = args.days
    else:
        effective_days = args.days

    # CSV mode: redirect logs to file in output dir
    global _LOG_FILE
    if args.output == "csv":
        os.makedirs(args.output_dir, exist_ok=True)
        _LOG_FILE = os.path.join(args.output_dir, "run.log")
    ...
    for code in codes:
        try:
            result = analyze_stock(code, effective_days)
            results.append(result)
        except Exception as e:
            errors.append({"code": code, "error": str(e), "type": type(e).__name__})

    # Write CSV files first (if csv mode)
    if args.output == "csv" and results:
        write_csv_output(results, args.output_dir,
                         analysis_date=datetime.now().strftime("%Y-%m-%d"))

    # Stdout output
    if args.output == "csv":
        # Compact status summary — data is in CSV files
        summary = {
            "status": "ok" if not errors else "partial",
            "total_requested": len(codes),
            "total_success": len(results),
            "errors": [{"code": e["code"], "error": e["error"]} for e in errors],
            "output_dir": os.path.abspath(args.output_dir),
            "files": (
                [f"{r['code']}_ohlcv.csv" for r in results]
                + (["analysis_summary.csv"] if results else [])
            ),
        }
        print(json.dumps(summary, ensure_ascii=False))
    else:
        # Full JSON output (stderr mode)
        output = {
            "analysis_date": datetime.now().strftime("%Y-%m-%d"),
