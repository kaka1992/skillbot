...
class TestBuiltinTools:
    ...
    def setup_class(cls):
        ToolRegistry.clear()
        ToolRegistry.discover("tests/tools/builtin")
    ...
    def test_groups(self):
        by_group = {t.name: t.group for t in ToolRegistry.list()}
    ...
    def test_json_extract_nested_key(self):
        ...
        assert result.data["value"] == "Bob"

    def test_json_extract_invalid_json(self):
        t = ToolRegistry.get("json_extract")
        import asyncio

        result = asyncio.run(t.execute({"json_str": "{not json}", "key_path": "x"}))
    ...
    def test_json_extract_missing_key(self):
        ...
        result = asyncio.run(t.execute({"json_str": json.dumps({"a": 1}), "key_path": "b"}))
        assert result.error is not None
    ...
    def test_file_read_success(self):
        ...
        try:
            ...
            assert result.data["text"] == "hello from file"
        finally:
            os.unlink(tmp)
