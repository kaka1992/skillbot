...
@register(
    name="single_file_tool",
    description="loaded via file path",
    parameters={"type": "object"}
)
async def single_file_tool(params: dict) -> ToolResult:
    return ToolResult(data={"ok": True})
""")
            new = ToolRegistry.discover(tool_file)
            assert len(new) == 1
            assert new[0].name == "single_file_tool"

    def test_discover_single_file_not_found(self):
        new = ToolRegistry.discover("/nonexistent/tool.py")
        assert new == []

    def test_discover_skips_broken_file(self):
        import os
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # valid file
            with open(os.path.join(tmp, "ok_tool.py"), "w") as f:
                f.write("""
from tools import ToolResult, register

@register(
    name="ok_tool",
    description="should load despite broken sibling",
    parameters={"type": "object"}
)
async def ok_tool(params: dict) -> ToolResult:
    return ToolResult(data={"ok": True})
""")
            new = ToolRegistry.discover(tmp)
            assert len(new) == 1
            assert new[0].name == "ok_tool"
