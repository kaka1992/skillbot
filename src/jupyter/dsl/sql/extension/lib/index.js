...
function parentsFor(doc, node) {
  for (let path = []; ; ) {
    ...
    if (!plainID(name2))
      return path;
    path.unshift(idName(doc, name2));
    node = tokenBefore(name2);
  }
}
function sourceContext(state, startPos) {
  let pos = (0, import_language.syntaxTree)(state).resolveInner(startPos, -1);
...
function completeKeywords(keywords2, upperCase, build) {
  ...
  return (0, import_autocomplete.ifNotIn)(["QuotedIdentifier", "String", "LineComment", "BlockComment", "."], (0, import_autocomplete.completeFromList)(completions));
}
var parser = /* @__PURE__ */ parser$1.configure({
  props: [
    /* @__PURE__ */ import_language.indentNodeProp.add({
      Statement: /* @__PURE__ */ (0, import_language.continuedIndent)()
    }),
...
var SQLDialect = class _SQLDialect {
  ...
  */
  static define(spec) {
    ...
    let language2 = import_language.LRLanguage.define({
      name: "sql",
...
function sql(config = {}) {
  ...
  return new import_language.LanguageSupport(lang.language, [
...
var sqlCompartment = new import_state.Compartment();
...
function applySqlLanguage(cell) {
  try {
    const cmView = cell.editor.editor;
    if (!cmView?.dispatch) return;
    cmView.dispatch({
      effects: sqlCompartment.reconfigure(
        import_language2.language.of(sql({ upperCaseKeywords: true }))
      )
    });
  } catch {
  }
}
function formatActiveCell(app) {
...
print(format_sql('''${escaped}'''), end='')`;
  session.kernel.requestExecute({ code }).done.then((reply) => {
    const formatted = reply?.content?.text || sqlBody;
...
function updateNotebook(panel) {
  panel.content.widgets.forEach((cell) => {
    if (cell.model.type === "code" && isSqlCell(cell)) {
