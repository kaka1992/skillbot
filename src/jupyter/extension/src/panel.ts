...
class AgentPanel extends Widget {
  ...
  private _ensureResponsePrefix(): void {
  ...
  private _renderPrompt(text: string): void {
  ...
  private _renderTool(name: string): void {
    this._ensureResponsePrefix();
  ...
  private _renderThinking(content: string): void {
    this._ensureResponsePrefix();
  ...
  private _renderCodeBlock(_language: string, code: string): void {
    this._ensureResponsePrefix();
    this._textEl = null;
  ...
  private _renderPlanBlock(text: string): void {
    this._ensureResponsePrefix();
    this._textEl = null;
    const wrapper = document.createElement('div');
  ...
  private _renderResult(summary: string): void {
    this._ensureResponsePrefix();
    this._textEl = null;
    const div = document.createElement('div');
  ...
  private _renderPlanConfirm(summary: string): void {
  ...
  private _getConfirmHint(): string {
    if (this._planConfirmFeedbackMode) {
  ...
  private _renderConfirmOptions(): void {
  ...
  private _sendConfirmToBackend(cmd: string): void {
    this._startBlock();
  ...
  private _submitPlanConfirm(): void {
    if (this._planConfirmFeedbackMode) {
      ...
      if (!feedback) {
        ...
        if (hint) {
          ...
          setTimeout(() => {
            ...
            if (h) h.textContent = this._getConfirmHint();
          }, 2000);
        }
        this._confirmWrapper.focus();
        return;
  ...
  private _cancelPlanConfirm(): void {
    this._closeConfirm();
    this._inputEl.focus();
    this._setStatus('○', 'idle');
    if (this._kernel) {
