...
export function renderPlanConfirm(panel: any, summary: string): void {
...
export function getConfirmHint(panel: any): string {
...
export function renderConfirmOptions(panel: any): void {
  // Save feedback text before re-render (innerHTML destroys old textarea)
  const oldTa = panel._confirmWrapper.querySelector('.skillbot-confirm-feedback') as HTMLTextAreaElement;
  ...
  const optionsHtml = options.map((label: string, i: number) => {
  ...
  if (savedFeedback) {
    const newTa = panel._confirmWrapper.querySelector('.skillbot-confirm-feedback') as HTMLTextAreaElement;
    if (newTa) newTa.value = savedFeedback;
  }

  if (panel._planConfirmFeedbackMode) {
    const ta = panel._confirmWrapper.querySelector('.skillbot-confirm-feedback') as HTMLTextAreaElement;
    if (ta) ta.focus();
  }
}

export function closeConfirm(panel: any): void {
...
export function sendConfirmToBackend(panel: any, cmd: string): void {
  panel._startBlock();
...
export function submitPlanConfirm(panel: any): void {
  if (panel._planConfirmFeedbackMode) {
    const ta = panel._confirmWrapper.querySelector('.skillbot-confirm-feedback') as HTMLTextAreaElement;
    ...
    if (!feedback) {
      ...
      if (hint) {
        ...
        if (newHint) newHint.textContent = 'Type feedback first, or select an option above';
        setTimeout(() => {
          const h = panel._confirmWrapper.querySelector('.skillbot-confirm-hint');
          if (h) h.textContent = getConfirmHint(panel);
        }, 2000);
      }
      panel._confirmWrapper.focus();
      return;
...
export function cancelPlanConfirm(panel: any): void {
  closeConfirm(panel);
  panel._inputEl.focus();
